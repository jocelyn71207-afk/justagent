...
- 側邊選單「共享資源庫」群組底下的子項目之一（另一個子項目是 3.11 知識庫管理），
  「共享資源庫」本身不是單一頁面，是側邊選單裡包住這兩個子功能的分組名稱
- 團隊層級的檔案管理
...
畫面是**單一導覽欄**（可在「文字+圖示」展開態／「純圖示」收合態之間切換，按鈕在使用者頭像列），團隊相關的項目收在獨立的「團隊選單」面板裡，**預設是展開的**；面板是版面裡正常的一欄（推擠右側內容，不是蓋在內容上面的浮層），點「團隊功能」可以收合／再展開，也可以點面板的 X 或點導覽欄以外的地方關閉。手機版（漢堡選單）維持原本的清單式抽屜，這次改版沒有動。

**導覽欄（單一欄，由上到下）**
- 使用者頭像/名稱/收合按鈕 → 頭像點擊有個人設定／登出
- 企業選擇器（點擊向下展開清單，目前僅一間企業，UI 已比照多企業情境設計）
- 探索（模組尚未整理進本文件，見 `src/views/Explore.vue`）
- 全域搜尋（展開時是常駐輸入框，收合時點擊圖示才彈出）
- 最近使用 → 3.1 ProjectDashboard（全域範圍，不分團隊）
- 企業/團隊設定 → 3.6 CompanyTeamSettings
- 團隊功能：停在任何團隊頁面時會亮起；點擊開關「團隊選單」面板——開關狀態只看使用者操作，不會因為網址帶 teamId 而有額外的自動開關
- 返回入口頁 → `/entrance`（AppEntrance）
- 底部：Logo、版本號

**團隊選單面板（預設展開，點「團隊功能」可收合）**
...
- 共享資源庫（可展開群組，非單一頁面）
  ...
  - 知識庫管理 → 3.11 KnowledgeBase
- 權限管理 → 3.5 TeamAccessManagement
...
- 待補：`src/views/KnowledgeBase.vue` 的詳細說明（含版本比對、AI 整理等功能）
