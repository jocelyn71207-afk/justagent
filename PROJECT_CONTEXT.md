...
畫面分兩塊，分類原則是「要不要先選定團隊」：
- **rail**（左側常駐圖示條）：不需要選定團隊即可使用的**全域**功能
- **side-panel**（選定團隊後才出現的常駐面板）：跟目前選定團隊有關的功能

手機版（漢堡選單）跟桌機版共用同一套模型與狀態：先選一個團隊，面板/選單只顯示這一個團隊的項目，用團隊切換器切換；不是每個團隊各自一個可展開表頭的手風琴。

**rail（全域）**
- 使用者頭像 → 個人設定 / 登出
- 企業選擇器（目前僅一間企業，UI 已比照多企業情境設計）
- 最近使用 → 3.1 ProjectDashboard（全域範圍，不分團隊）
- 探索（模組尚未整理進本文件，見 `src/views/Explore.vue`）
- 全域搜尋
- 團隊入口：跨團隊頁面（最近使用／探索）沒有常駐面板可點時，用這個彈出團隊清單快速切換
