...
畫面是**單一導覽欄**（可在「文字+圖示」展開態／「純圖示」收合態之間切換，按鈕在使用者頭像列），團隊相關的項目收在獨立的「團隊選單」彈出面板裡，只有點擊導覽欄的「團隊功能」項目才會出現，不是常駐展開；點面板的 X 或背景可以關閉。手機版（漢堡選單）維持原本的清單式抽屜，這次改版沒有動。
...
- 團隊功能：停在任何團隊頁面時會亮起；點擊開關「團隊選單」面板——不會因為網址帶 teamId 就自動彈出，只有主動點擊才會開
