...
**審核佇列**（管理區 tab，`SkillReviewCard` 清單）：這些卡片本身內容量就不同（可展開的技能指令、選填的 AI 分析區塊），目前的格線可能強制等高，讓收合狀態的卡片看起來空。改為**自然高度對齊**（`align-items: start`，不強制 stretch），讓卡片高度反映真實內容量。套用活潑感系統。
